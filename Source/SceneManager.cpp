...
void SceneManager::SetupSceneLights()
{
	...
	// Main white light, soft overhead light for the workstation
	...
	// Secondary warm desk lamp light, brighter and more focused under the shade
	m_pShaderManager->setVec3Value("lightSources[1].position", 7.2f, 2.1f, -1.8f);
	m_pShaderManager->setVec3Value("lightSources[1].ambientColor", 0.03f, 0.02f, 0.01f);
	m_pShaderManager->setVec3Value("lightSources[1].diffuseColor", 1.00f, 0.85f, 0.45f);
	m_pShaderManager->setVec3Value("lightSources[1].specularColor", 0.70f, 0.55f, 0.25f);
	m_pShaderManager->setFloatValue("lightSources[1].focalStrength", 22.0f);
...
void SceneManager::PrepareScene()
{
	...
	m_basicMeshes->LoadConeMesh(); // added cone mesh for the desk lamp shade
}
...
void SceneManager::RenderScene()
{
	...
	/******************************************************************/
	// Lamp base, circular and moved farther to the right
	scaleXYZ = glm::vec3(1.1f, 0.12f, 1.1f);

	XrotationDegrees = 0.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;

	positionXYZ = glm::vec3(7.2f, 0.35f, -1.8f);
	...
	// set lamp base color to dark gray
	...
	/******************************************************************/
	// Lamp stem, cylinder connecting the base and lampshade
	scaleXYZ = glm::vec3(0.18f, 3.35f, 0.18f);

	XrotationDegrees = 0.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;

	positionXYZ = glm::vec3(7.2f, 0.60f, -1.8f);
	...
	// set lamp stem color to light gray
	...
	/******************************************************************/
	// Lamp shade, larger cone on top of the stem
	scaleXYZ = glm::vec3(1.45f, 1.35f, 1.45f); // made the cone bigger

	XrotationDegrees = 0.0f; 
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;

	positionXYZ = glm::vec3(7.2f, 2.75f, -1.8f);
	...
	// set lamp shade color to warm yellow
	SetShaderColor(0.98f, 0.86f, 0.35f, 1.0f);
